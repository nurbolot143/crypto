export { default as benefitsIcon1 } from "./benefits_icon1.svg";
export { default as benefitsIcon2 } from "./benefits_icon2.svg";
export { default as benefitsIcon3 } from "./benefits_icon3.svg";
export { default as benefitsIcon4 } from "./benefits_icon4.svg";

export { default as partner1 } from "./partner1.svg";
export { default as partner2 } from "./partner2.svg";
export { default as partner3 } from "./partner3.svg";
export { default as partner4 } from "./partner4.svg";
export { default as partner5 } from "./partner5.svg";
export { default as partner6 } from "./partner6.svg";

export { default as android } from "./android.svg";
export { default as apple } from "./apple.svg";
export { default as linux } from "./linux.svg";
export { default as windows } from "./windows-8.svg";

export { default as globalNetwork1 } from "./globalNetwork_icon1.svg";
export { default as globalNetwork2 } from "./globalNetwork_icon2.svg";
export { default as globalNetwork3 } from "./globalNetwork_icon3.svg";
